...
const getAiFeedback = async (analysisData) => {
  ...
  const { resumeText, jobTitle, jobDescription, matchedSkills, missingSkills, scoreBreakdown } = analysisData;
  ...
  const prompt = `
    ...
    RESUME TEXT: ${resumeText.substring(0, 4000)} // Limit text size for token efficiency
  ...
  try {
    const response = await openai.chat.completions.create({
      model: "gpt-4o-mini", // Cost efficient and capable
      messages: [
        { role: "system", content: "You are a professional career coach. Respond only in JSON format." },
...
const getFallbackFeedback = (data) => {
  return {
    overallFeedback: "Your resume shows some relevant experience, but there is room for optimization to better align with the job requirements. Focus on incorporating the missing skills and quantifying your achievements.",
    topIssues: [
    ...
    sectionFeedback: {
      summary: "Ensure your summary highlights your most relevant achievements for this specific role.",
      skills: "Organize your skills into categories and include all technical tools mentioned in the JD that you know.",
      experience: "Use action verbs and quantify your results (e.g., increased sales by 20%).",
      projects: "Focus on projects that demonstrate the core technologies required for this job.",
      education: "Ensure your degree and relevant certifications are clearly listed."
    },
    rewriteSuggestions: [
      {
        "original": "Responsible for developing web applications.",
        "improved": "Developed and maintained high-performance web applications using React and Node.js, improving load times by 15%.",
